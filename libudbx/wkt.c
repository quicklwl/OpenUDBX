...
static int wkt_data(const geom_consumer_t *consumer, const geom_header_t *header, size_t data_count, const char *pdata,errorstream_t *error) {
	int result = SQLITE_OK;

	wkt_writer_t *writer = (wkt_writer_t *)consumer;
	strbuf_append(&writer->strbuf, " \"");
	result = strbuf_append(&writer->strbuf, pdata);
	strbuf_append(&writer->strbuf, "\"");
	if (result != SQLITE_OK) {
		goto exit;
	}
...
int wkt_writer_init(wkt_writer_t *writer) {
  geom_consumer_init(&writer->geom_consumer, NULL, NULL, wkt_begin_geometry, wkt_end_geometry, wkt_coordinates,wkt_data);
...
static int wkt_read_annotation_text(wkt_tokenizer_t *tok, const geom_header_t *header, const geom_consumer_t *consumer, errorstream_t *error) {
	if (tok->token == WKT_EMPTY) {
		wkt_tokenizer_next(tok);
		return SQLITE_OK;
	}
