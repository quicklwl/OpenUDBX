...
int gpb_write_header(binstream_t *stream, geom_blob_header_t *gpb, geom_type_t geom_type, errorstream_t *error) {
  CHECK_ENV(gpb, error)
...
static int gpb_data(const geom_consumer_t *consumer, const geom_header_t *header, size_t data_count, const char *pdata,  errorstream_t *error) {
	...
	wkb_writer_t *wkb = &writer->wkb_writer;
	geom_consumer_t *wkb_consumer = wkb_writer_geom_consumer(wkb);
	result = wkb_consumer->data(wkb_consumer, header, data_count, pdata, error);
	if (result != SQLITE_OK) {
		goto exit;
	}
exit:
...
static int gpb_end(const geom_consumer_t *consumer, errorstream_t *error) {
  ...
  result = gpb_write_header(stream, &writer->header,writer->geom_type, NULL);
  if (result != SQLITE_OK) {
    goto exit;
  }
...
int gpb_writer_init(geom_blob_writer_t *writer, int32_t srid) {
  geom_consumer_init(&writer->geom_consumer, NULL, gpb_end, gpb_begin_geometry, gpb_end_geometry, gpb_coordinates,gpb_data);
