...
int wkb_fill_envelope(binstream_t *stream, wkb_dialect dialect, geom_envelope_t *envelope, errorstream_t *error) {
  ...
  geom_consumer_init(&fill_gpb.consumer, NULL, NULL, NULL, NULL, fill_envelope_coordinates,NULL);
...
static int read_annotation(binstream_t *stream, wkb_dialect dialect, const geom_consumer_t *consumer, const geom_header_t *header, errorstream_t *error) {
	uint32_t point_count;
	if (binstream_read_u32(stream, &point_count) != SQLITE_OK) {
		if (error) {
			error_append(error, "Error reading line string point count");
...
static int wkb_data(const geom_consumer_t *consumer, const geom_header_t *header, size_t data_count, const char *pdata, errorstream_t *error) {
	int result = SQLITE_OK;

	wkb_writer_t *writer = (wkb_writer_t *)consumer;
	binstream_t *stream = &writer->stream;

	result = binstream_write_u32(stream,data_count);
	result = binstream_write_nu8(stream,pdata, data_count);
	if (result != SQLITE_OK) {
		goto exit;
	}
...
int wkb_writer_init(wkb_writer_t *writer, wkb_dialect dialect) {
  geom_consumer_init(&writer->geom_consumer, NULL, wkb_end, wkb_begin_geometry, wkb_end_geometry, wkb_coordinates,wkb_data);
